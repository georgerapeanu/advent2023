...
let input_to_matrix list_of_lines =
  Array.of_list
  @@ List.map list_of_lines ~f:(fun line ->
         Array.of_list @@ String.to_list line )

let is_digit c =
  Char.to_int '0' <= Char.to_int c && Char.to_int c <= Char.to_int '9'

let is_operator c = (not (Char.equal c '.')) && not (is_digit c)

let is_star c = Char.equal c '*'

let get_number_at_pos (matrix : char array array) i j =
  if j > 0 && is_digit matrix.(i).(j - 1) then None
  else
    let k = ref j in
    ...
    let number_string =
      Array.sub matrix.(i) ~pos:j ~len:(!k - j) |> String.of_array
    in
    if String.length number_string = 0 then None
    else
          ...
          for dy = -1 to 1 do
            ...
            if
              (not
                 ...
                 || y < 0
                 || y >= Array.length matrix.(x) ) )
              && is_operator matrix.(x).(y)
            then answer := Option.some @@ Int.of_string number_string
          done
        done
      done ;
      !answer
...
let get_number_start matrix i j =
  if not (is_digit matrix.(i).(j)) then (i, j)
  else
    let k = ref j in
...
let get_numbers_adjacent_to_star matrix i j =
  if is_star matrix.(i).(j) then
    ...
    let unique_numbers_list_opt =
      List.map
        ~f:(fun x ->
          let f, s = x in
          get_number_at_pos matrix f s )
        unique_starts_list
    in
    let unique_numbers_list =
      List.filter ~f:Option.is_some unique_numbers_list_opt
...
let part1 () =
  ...
  let list_of_num : int list =
    List.filter !list_of_num_opt ~f:Option.is_some
    |> List.map ~f:(fun x -> Option.value_exn x)
    |> List.rev
  in
...
let part2 () =
  ...
  for i = 0 to Array.length matrix - 1 do
    for j = 0 to Array.length matrix.(i) - 1 do
      list_of_gear_stuff :=
        get_numbers_adjacent_to_star matrix i j :: !list_of_gear_stuff
    done
  done ;
  let sum =
    List.filter !list_of_gear_stuff ~f:(fun x -> List.length x = 2)
